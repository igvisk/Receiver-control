...
def show_settings():
    ...
    ip_entry.insert(0, "192.168.88.110")
    ip_entry.pack(pady=5)

    # Tlačidlo
    ttk.Button(settings_frame, text="Pripojiť", command=on_connect).pack(pady=10)

    # Status label (ttk verzia so štýlom)
    status_label = ttk.Label(settings_frame, text="Status", style="Status.TLabel")
...
def update_status_label(status: str):
    if status == 'ON':
        status_label.config(text='ON', style="On.TLabel")
    elif status == 'STANDBY':
        status_label.config(text='STANDBY', style="Standby.TLabel")
    elif status == 'UNKNOWN':
        status_label.config(text='NOT AVAILABLE', style="Unknown.TLabel")
    else:
        status_label.config(text='NOT FOUND', style="Error.TLabel")



def on_connect():
    global ip_entry, status_label
    if ip_entry is None or status_label is None:
        print("IP entry or status label not initialized.")
        return

    ip = ip_entry.get()
    status_label.config(text='Kontrolujem...', style="Checking.TLabel")
...
style.configure("Status.TFrame", background="pink")
